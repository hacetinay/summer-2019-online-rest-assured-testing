...
@Test
    public void test2_1(){
                ...
                when().
                get("/countries").then().assertThat().statusCode(200).
                and().assertThat().contentType(ContentType.JSON).
                and().assertThat().body("items[0].country_name",is("United States of America")).
                log().all(true);
//queryParam("q", "{\"country_id\":\"US\"}");
...
public void test4(){
    ...
    @DisplayName("verify that body returns following salary information after sorting from higher to lower(after sorting it in descending order)")
        ...
        List<Integer> actualSalaries=   given().accept(ContentType.JSON).when().
            get("/employees").thenReturn().jsonPath().get("items.salary");
//        Collections.sort(actualSalaries);
